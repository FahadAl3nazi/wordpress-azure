...
if (!class_exists('KEYDESIGN_ADDON_CLASS')) {
	...
	function initiate_keydesign_addon() {
		if( defined('WPB_VC_VERSION') ){
			if( version_compare( 5.7, WPB_VC_VERSION, '>' )){
	...
	function keydesign_version_notice() {
		...
		if( ( $is_multisite && $is_network_admin ) || !$is_multisite ) {
			echo '<div class="updated">
				<p>'.__('The','keydesign').' <strong>Keydesign Addon</strong> '.__('plugin requires','keydesign').' <strong>WPBakery Page Builder</strong> '.__('version 5.7 or greater.','keydesign').'</p>
			</div>';
		}
	}
	...
	function keydesign_addon_activate() {
		update_option('keydesign_addon_version', '2.8' );
	}
