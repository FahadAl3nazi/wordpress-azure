<?php
  ...
  $title_align_class = $single_page_title_align = $values = $titlebar_bg = $shop_titlebar_bg = $shop_title_bar_text_align = $shop_page_particles = '';
...
<?php elseif ( class_exists( 'bbPress' ) && is_bbpress() ) : ?>
  ...
  <?php elseif (class_exists( 'WooCommerce' ) && is_woocommerce()) : ?>
    <?php if ( empty($shop_page_showhide_title_section)) : ?>
      <header class="entry-header blog-header shop-header <?php if (isset($redux_ThemeTek['tek-topbar'])) { if ($redux_ThemeTek['tek-topbar'] == '1') { echo esc_html('with-topbar'); }} ?>" <?php if ($shop_titlebar_bg != '') { echo 'style="background-color: '.$shop_titlebar_bg.';"'; } ?>>
     ...
     <div class="row blog-page-heading <?php echo esc_html($title_align_class); ?>">
        ...
        <div class="container">
          ...
          <?php elseif ( is_search() ) : ?>
            <h2 class="section-heading">
               <?php echo apply_filters( 'kd_search_query_title', esc_html__("Search results for", "leadengine") ); ?>: <?php the_search_query();  ?>
            </h2>
          <?php elseif ( is_category() ) : ?>
            <h2 class="section-heading">
               <?php echo apply_filters( 'kd_single_cat_title', esc_html__("Currently browsing", "leadengine") ); ?>: <?php single_cat_title(); ?>
            </h2>
          <?php elseif ( is_tag() ) : ?>
            <h2 class="section-heading">
               <?php echo apply_filters( 'kd_single_tag_title', esc_html__("All posts tagged", "leadengine") ); ?>: <?php single_tag_title(); ?>
            </h2>
          <?php elseif ( is_author() ) : ?>
            <h2 class="section-heading">
               <?php echo apply_filters( 'kd_author_archive_title', esc_html__("All posts by", "leadengine") ); ?> <?php echo esc_attr(get_userdata(get_query_var('author'))->display_name); ?>
            </h2>
          <?php elseif ( is_day() ) : ?>
            <h2 class="section-heading">
               <?php echo apply_filters( 'kd_day_archive_title', esc_html__("Posts archive for", "leadengine") ); ?> <?php echo get_the_date('F jS, Y'); ?>
            </h2>
          <?php elseif ( is_month() ) : ?>
            <h2 class="section-heading">
               <?php echo apply_filters( 'kd_month_archive_title', esc_html__("Posts archive for", "leadengine") ); ?> <?php echo get_the_date('F, Y'); ?>
            </h2>
          <?php elseif ( is_year() ) : ?>
            <h2 class="section-heading">
              <?php echo apply_filters( 'kd_year_archive_title', esc_html__("Posts archive for", "leadengine") ); ?> <?php echo get_the_date('Y'); ?>
            </h2>
